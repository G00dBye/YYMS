...
/**
 * Artifact from Invictus that improved the old MapleAESOFB by making it easily
 * scaled with session groups while using Apache's MINA library. Ported over for
 * usage within a Netty setup since it will work exactly the same.
 *
...
public final class MapleCrypto {
     ...
     * initialized at the starting part of the program by providing the current
     * version the server is running.
     *
     * @see MapleAES#checkPacket(byte[], byte[]) checking
     * net.swordie.ms.connection.packet header using these.
     * @see MapleAES#checkPacket(int, byte[]) checking
     * net.swordie.ms.connection.packet header using these.
     * @see MapleAES#initialize(short) where the values are initialized.
     */
    private static short gVersion, sVersion, rVersion;
    /**
     * Used for renewing the cryptography seed for sending or receiving packets.
     *
     */
    ...
    /**
     * Constructor for MapleAES. Creates the cipher that will be used for the
     * cryptography segment as well as the lock used for the same segment.
     */
    public MapleCrypto() {
        cipher = new AES();
    ...
    /**
     * Initializes the send and receive version values for
     * net.swordie.ms.connection.packet checking using the current version.
     *
    ...
    /**
     * Creates a header for the new net.swordie.ms.connection.packet to be sent
     * to the opposite end of the session (Server to Channel for this engine).
     * Contains information about the net.swordie.ms.connection.packet: whether
     * or not it is valid and the actual length of the
     * net.swordie.ms.connection.packet.
     *
     * @param delta the input net.swordie.ms.connection.packet length before
     * adding the header.
    ...
    /**
     * Gets the length of the net.swordie.ms.connection.packet given the
     * received header.
     *
     * @param delta the net.swordie.ms.connection.packet header to be used to
     * find the net.swordie.ms.connection.packet length.
     *
    ...
    /**
     * Checks the net.swordie.ms.connection.packet to make sure it is valid and
     * that the session between the net.swordie.ms.client and server is secure
     * and legitimate.
     *
     * @param delta the net.swordie.ms.connection.packet header from the
     * received net.swordie.ms.connection.packet (4 bytes in length).
     * @param gamma the current receive seed.
     *
     * @return whether or not the net.swordie.ms.connection.packet is valid
     * (consequently, if not valid, the session is terminated usually).
     */
...
//        System.out.println("Delta[1] ^ gamma[3] = " + Integer.toBinaryString((delta[1] ^ gamma[3]) & 0xFF));
     ...
     * except that we convert the integer net.swordie.ms.connection.packet
     * header (4 bytes combined) into an array of the first 2 bytes of the
     * integer net.swordie.ms.connection.packet header.
    ...
    /**
     * Shuffles the seed (or IV) used for this cryptography stage. Almost always
     * called when this cryptography stage is finished since rolling the seed is
     * important to keeping a valid session.
     *
     * @param delta the old seed or IV to be changed into the new one.
     *
     * @return the new seed or IV to be used for this stage of cryptography for
     * the next net.swordie.ms.connection.packet sent or received.
     */
