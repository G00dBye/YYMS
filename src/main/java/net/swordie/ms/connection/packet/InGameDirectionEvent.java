...
/**
 * InGameDirectionEvent, for when a client is in InGameDirection mode (by using
 * {@link net.swordie.ms.connection.packet.UserLocal(boolean)})
 *
 * @author Sjonnie Created on 8/31/2018.
 */
public class InGameDirectionEvent implements Encodable {
     ...
     * @param pos position the camera should move to (not applicable if back is
     * true)
     * @return the event
     */
     ...
     *
     * @param npcID the npc's id on which the camera should focus on, or 0 if
     * the camera should focus on the player
     * @return the event
     */
