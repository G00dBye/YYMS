...
//        monsterBattleRankInfo = new MonsterBattleRankInfo();
    ...
    /**
     * Encodes this Char's info inside a given {@link OutPacket}, with given
     * info.
     *
...
//            getReturnEffectInfo().encode(outPacket); // ReturnEffectInfo::Decode
    ...
    /**
     * Sets the job of this Char with a given id. Does nothing if the id is
     * invalid. If it is valid, will set this Char's job, add all Skills that
     * the job should have by default, and sends the info to the client.
     *
     * @param id
     */
    ...
    /**
     * Adds a {@link Skill} to this Char. Changes the old Skill if the Char
     * already has a Skill with the same id. Removes the skill if the given
     * skill's id is 0.
     *
     ...
     *
     * @param skill The Skill this Char should get.
     * @param addRegardlessOfLevel if this is true, the skill will not be
     * removed from the char, even if the cur level of the given skill is 0.
     */
    ...
    /**
     * Removes a Skill from this Char. Sends change skill record to remove the
     * skill from the client.
     *
     * @param skillID the id of the skill that should be removed
     */
    ...
    /**
     * Initializes the BaseStat cache, by going through all the needed passive
     * stat changers.
     */
     ...
     * @return The Skill corresponding to the given id of this Char, or null if
     * there is none.
     */
    ...
    /**
     * Gets a {@link Skill} with a given ID. If <code>createIfNull</code> is
     * true, creates the Skill if it doesn't exist yet. If it is false, will
     * return null if this Char does not have the given Skill.
     *
     * @param id The id of the requested Skill.
     * @param createIfNull Whether or not this method should create the Skill if
     * it doesn't exist.
     * @return The Skill that the Char has, or <code>null</code> if there is no
     * such skill and <code>createIfNull</code> is false.
     */
    ...
    /**
     * Notifies all groups (such as party, guild) about all your changes, such
     * as level and job.
     */
    ...
    /**
     * Gets a raw Stat from this Char, unaffected by things such as equips and
     * skills.
     *
    ...
    /**
     * Adds a Stat to this Char, and immediately sends the packet to the client
    ...
    /**
     * Sends a message to this Char with a default colour
     * {@link ChatType#SystemNotice}.
     *
     * @param msg The message to display.
     */
    ...
    /**
     * Unequips an {@link Item}. Ensures that the hairEquips and both
     * inventories get updated.
     *
    ...
    /**
     * Equips an {@link Item}. Ensures that the hairEquips and both inventories
     * get updated.
     *
     * @param item The Item to equip.
     */
    ...
    /**
     * Creates a {@link Rect} with regard to this character. Adds all values to
     * this Char's position.
     *
     ...
     * @return The Equip corresponding to <code>bodyPart</code>. Null if there
     * is none.
     */
    ...
    /**
     * Returns a {@link Field} based on the current {@link FieldInstanceType} of
     * this Char (channel, expedition, party or solo).
     *
    ...
    /**
     * Warps this character to a given field, at the starting position. See
     * {@link #warp(Field, Portal) warp}.
     *
    ...
    /**
     * Warps this Char to a given {@link Field}, with the Field's "sp" portal as
     * spawn position.
     *
     * @param toField The Field to warp to.
    ...
    /**
     * Warps this Char to a given {@link Field} and {@link Portal}. Will not
    ...
    /**
     * Warps this character to a given field, at a given portal. Ensures that
     * the previous map does not contain this Char anymore, and that the new
     * field does. Ensures that all Lifes are immediately spawned for the new
     * player.
     *
    ...
    /**
     * Adds a given amount of exp to this Char. Immediately checks for level-up
     * possibility, and sends the updated stats to the client. Allows
     * multi-leveling.
     *
    ...
    /**
     * Adds exp to this Char. Will calculate the extra exp gained from buffs and
     * the exp rate of the server. Also takes an argument to show this info to
     * the client. Will not send anything if this argument (eii) is null.
     *
     * @param amount The amount of exp to add
    ...
    /**
     * Adds a given amount of exp to this Char, however it does not display the
     * Exp Message. Immediately checks for level-up possibility, and sends the
     * updated stats to the client. Allows multi-leveling.
     *
     * @param amount The amount of exp to add.
     */
    ...
    /**
     * Zero only. Goes into Beta form if Alpha, and into Alpha if Beta.
     */
...
//        write(WvsContext.zeroInfo(currentInfo));
    ...
    /**
     * Consumes a single {@link Item} from this Char's {@link Inventory}. Will
     * remove the Item if it has a quantity of 1.
     *
     * @param item The Item to consume, which is currently in the Char's
     * inventory.
     */
    ...
    /**
     * Consumes an item of this Char with the given id. Will do nothing if the
     * Char doesn't have the Item. Only works for non-Equip (i.e., type is not
     * EQUIPPED or EQUIP, CASH is fine) items. Calls {@link #consumeItem(Item)}
     * if an Item is found.
     *
    ...
    /**
     * Returns the current Set of Fields that this Char holds as personal
     * instances.
     *
    ...
    /**
     * Checks if this Char can hold an Item in their inventory, assuming that
     * its quantity is 1.
     *
    ...
    /**
     * Recursive function that checks if this Char can hold a list of items in
     * their inventory.
     *
    ...
    /**
     * Returns the total list of friends of this Char + the owning Account's
     * friends.
     *
    ...
    /**
     * Gets the current amount of a given stat the character has. Includes
     * things such as skills, items, etc...
     *
    ...
    /**
     * Gets a total list of basic stats that a character has, including from
     * skills, items, etc...
     *
    ...
    /**
     * Sets whether or not this user has chosen to use up an item to protect
     * their buffs upon next respawn.
     *
    ...
    /**
     * Returns whether this user has chosen to activate a buff protector for
     * their next respawn.
     *
     ...
     *
     * @return the Item the user has for prtoecting buffs, or null if there is
     * none.
     */
    ...
    /**
     * Resets the combo kill's timer. Interrupts the previous timer if there was
     * one.
     */
    ...
    /**
     * Checks if a skill is allowed to be cast, according to its cooltime. If it
     * is allowed, it immediately sets the cooltime and stores the next moment
     * where the skill is allowed. Skills without cooltime are always allowed.
     *
    ...
    /**
     * Sets a skill's cooltime according to their property in the WZ files, and
     * stores the moment where the skill comes off of cooldown.
     *
    ...
    /**
     * Adds honor exp to this Char, and sends a packet to the client with the
     * new honor exp. Honor exp added may be negative, but the total honor exp
    ...
    /**
     * Adds a skill to this Char. If the Char already has this skill, just
     * changes the levels.
     *
    ...
    /**
     * Checks if this Char has any of the given quests in progress. Also true if
     * the size of the given set is 0.
     *
     ...
     * @param slv the current skill level
     * @return whether the consumption was successful (unsuccessful = not enough
     * mp)
     */
    ...
    /**
     * Shows tutor automated message (the client is taking the message
     * information from wz).
     *
    ...
    public void removePsychicArea(int psychicAreaKey) {
        this.psychicArea.remove(psychicAreaKey);
    }
