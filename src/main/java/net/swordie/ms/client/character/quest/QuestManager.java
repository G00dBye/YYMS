...
@MapKeyColumn(name="<name_of_map_key_in_table>")
    ...
    /**
     * Checks if a quest is complete. This means that a quest's status is
     * STARTED, and that all the requirements to complete it have been met.
     *
     * @param questID the quest's id to check
     * @return completeness
     */
    ...
    /**
     * Adds a new {@link Quest} to this QuestManager's quests. If it already
     * exists, doesn't do anything. Use {@link #replaceQuest(Quest)} if a given
     * quest should be overridden.
     *
    ...
    /**
     * Adds a new {@link Quest} to this QuestManager's quest. If it already
     * exists, overrides the old one with the new one.
     *
    ...
    /**
     * Completes a quest. Assumes the check for in-progressness has already been
     * done, so this method can be used to complete quests that the Char does
     * not actually have.
     *
    ...
    /**
     * Removes a given quest from this QuestManager, and notifies the client of
     * this change. Does nothing if the Char does not currently have the quest.
     *
    ...
    /**
     * Adds a quest to this QuestManager with a given id. If there is no quest
     * with that id, does nothing.
     *
