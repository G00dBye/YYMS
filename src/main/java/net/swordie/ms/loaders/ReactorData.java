...
public class ReactorData {
    ...
    private static void loadReactorsFromWZ() {
        ...
        for (File file : files) {
            ...
            if (questNode != null) {
                for (Node intNode : XMLApi.getAllChildren(questNode)) {
                    //String name = XMLApi.getNamedAttribute(intNode, "name");
                    //String value = XMLApi.getNamedAttribute(intNode, "value");
                    //国服quest节点有子节点，不止一个任务ID会出现错误，如id 1022002，要遍历一下子节点
                    ri.setQuest(Integer.parseInt(XMLApi.getNamedAttribute(intNode, "value")));
