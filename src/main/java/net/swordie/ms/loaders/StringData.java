...
public class StringData {
    ...
    public static void loadQuestStringsFromWz() {
        ...
        for (Node topNode : nodes) {
            for (Node mainNode : XMLApi.getAllChildren(topNode)) {
                ...
                for (Node infoNode : XMLApi.getAllChildren(mainNode)) {
                    ...
                    switch (name) {
                        case "name":
                            getQuestStrings().put(id, value);
                            break;
                    }
                }
            }
        }
        log.info(String.format("加载 %d 个任务信息，用时%d毫秒", getQuestStrings().size(), System.currentTimeMillis() - start));
    }
    ...
    private static void saveQuestStrings(String dir) {
        ...
        try (DataOutputStream dataOutputStream = new DataOutputStream(new FileOutputStream(file))) {
            dataOutputStream.writeInt(getQuestStrings().size());
            for (Map.Entry<Integer, String> entry : getQuestStrings().entrySet()) {
                int id = entry.getKey();
    ...
    public static void generateDatFiles() {
        ...
        log.info(String.format("【完成】生成 String 数据，用时%d毫秒。", System.currentTimeMillis() - start));
    }
